    ...
    const LinuxRtspCameraConfig& config)
{
    ...
    std::shared_ptr<Session> reusedSession;
    {
    ...
    if (reusedSession) {
        ...
        std::filesystem::path clipDirectory;
        {
            std::lock_guard<std::mutex> processLock(reusedSession->processMutex);
            activeSource = reusedSession->activeSource.empty()
                ? reusedSession->source
                : reusedSession->activeSource;
            thumbnailPath = reusedSession->thumbnailPath;
            clipDirectory = reusedSession->clipDirectory;
        }

        const bool hadThumbnail = IsNonEmptyRegularFile(thumbnailPath);
        std::string refreshError;
        bool refreshedThumbnail = false;
        if (!activeSource.empty() && !thumbnailPath.empty()) {
            refreshedThumbnail =
                RunThumbnailProbe(activeSource, thumbnailPath, ReadStartFrameTimeoutMs(), refreshError);
        }
        const bool hasThumbnail = refreshedThumbnail || hadThumbnail || IsNonEmptyRegularFile(thumbnailPath);
        if (!refreshedThumbnail && logger_ != nullptr && !refreshError.empty()) {
            logger_->warn(
                "camera",
                "Linux camera thumbnail refresh failed for reused camera " +
                    config.cameraId + ": " + refreshError);
        }

        result.started = true;
        result.reused = true;
        result.thumbnailGenerated = hasThumbnail;
        ...
        result.activeRtspUrlMasked = IsRtspSource(activeSource)
            ? MaskLinuxRtspCredentials(activeSource)
            : std::string();
        result.candidateCount = static_cast<int>(reusedSession->candidates.size());
        result.reconnectAttempts = reusedSession->reconnectAttempts.load();
        result.reconnecting = reusedSession->reconnecting.load();
        result.thumbnailPath = thumbnailPath.string();
        result.clipDirectory = clipDirectory.string();
        result.error = hasThumbnail
            ? reusedSession->lastError
            : (!refreshError.empty() ? refreshError : reusedSession->lastError);
...
nlohmann::json LinuxCameraSessionManager::statusJson() const
{
    ...
    nlohmann::json cameraNames = nlohmann::json::object();
    nlohmann::json connectionMethods = nlohmann::json::object();
    nlohmann::json activeSources = nlohmann::json::object();
    ...
    for (const auto& item : sessions_) {
        ...
        cameraNames[cameraId] = session->config.cameraName;
        connectionMethods[cameraId] = session->config.connectionMethod;
        activeSources[cameraId] = IsRtspSource(session->activeSource)
            ? MaskLinuxRtspCredentials(session->activeSource)
    ...
    return {
        ...
        { "camera_name_by_camera", cameraNames },
        { "connection_method_by_camera", connectionMethods },
        { "active_source_by_camera", activeSources },
