...
export function useCameraEvents(cameras: any[], onCameraStateChange?: () => void) {
  ...
  useEffect(() => {
    ...
    pollingManager.register("job-step-start-blocked-events-hook", {
      url,
      interval: 4000,
      jitterMaxMs: 500,
      onData: (events: CameraEvent[]) => {
      ...
      onError: (error) => {
        console.error("[useCameraEvents] Job step start blocked polling error:", error);
      },
    });

    return () => {
      pollingManager.unregister("job-step-start-blocked-events-hook");
    };
