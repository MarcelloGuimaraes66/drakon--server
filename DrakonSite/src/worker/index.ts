...
app.post("/api/agent/chat-context", async (c) => {
  await ensureSchema(c.env.DB);
  const url = new URL(c.req.url);
  const clientId = url.searchParams.get("client_id");
