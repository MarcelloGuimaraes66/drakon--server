...
export async function ensureCentralIdentitySchema(db: D1Database): Promise<void> {
  ...
  const initPromise = (async () => {
    ...
    await addColumnIfMissing(`ALTER TABLE server_users ADD COLUMN is_active INTEGER NOT NULL DEFAULT 1`);
    await addColumnIfMissing(`ALTER TABLE server_users ADD COLUMN auth_version INTEGER NOT NULL DEFAULT 1`);
